 ...
 *    and `dsh-llm-pi-ai` deliberately strips any profile header colliding with
 ...
 */
const AGENTROUTER_SETTINGS_NAMESPACE = settingsNamespace('llm-agentrouter')
...
const Config = z.object({
   ...
   * rather than a constant: the relay may pin a different CLI build later.
   */
  userAgent: z
    .string()
 ...
 *
 * The wrapper never reads or rebuilds a body: for the shape every provider SDK
 * in this harness uses (url plus an init object) it copies `init` and replaces
 * only its headers. A bare `Request` is re-created with `duplex: 'half'` so a
 * streamed body survives the clone.
 *
...
function fenceFetch(native, current) {
  return function agentRouterFetch(input, init) {
    ...
    if (isRequest) return native(new Request(target, { ...(init ?? {}), ...requestInitOf(input), headers, duplex: 'half' }))
    return native(target, { ...init, headers })
  }
}
...
function apply(ctx, config) {
  ...
  let current = () => config
  installSettingsSection(ctx, AGENTROUTER_SETTINGS_NAMESPACE, Config, config, {
    setSource: (source) => {
      current = source
    },
    onChange: () => {},
