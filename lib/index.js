 ...
 *
 * 3. **Tool schemas.** The relay balances one model across several upstream
...
function fenceFetch(native, current) {
  return function agentRouterFetch(input, init) {
    ...
    const rewritten =
      typeof init?.body === 'string' ? relayToolSchemaPatch(init.body) : undefined
    ...
    if (destination.toLowerCase() === url.host.toLowerCase()) {
      ...
      else pending = native(input, { ...init, ...(rewritten === undefined ? {} : { body: rewritten }), headers })
    } else {
      const target = new URL(url)
      target.host = destination
      if (isRequest) pending = native(new Request(target, { ...(init ?? {}), ...requestInitOf(input), headers, duplex: 'half' }))
      else pending = native(target, { ...init, ...(rewritten === undefined ? {} : { body: rewritten }), headers })
    }
